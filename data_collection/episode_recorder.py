...
def build_preview(cam1_bgr, cam2_bgr, rec_state, n_steps, avg_hz, prompt,
    ...
    try:
        while rclpy.ok():
            ...
            if key == ord('p') or key == ord('P'):
                ...
                if new_prompt:
                    ...
                    node.get_logger().warning(
                        "[Recorder] Empty input — prompt unchanged.")
                cv2.namedWindow(WIN, cv2.WINDOW_NORMAL)
                cv2.resizeWindow(WIN, PREVIEW_W * 2, PREVIEW_H)
            ...
            # ── B — Begin Recording (WAITING only) ────────────────────────────
                ...
                if not prompt:
                    node.get_logger().warning(
                        "[Recorder] No prompt set — press P first!")
                    ...
                    node.get_logger().info(
                        f"[Recorder] *** BEGIN episode {episode_idx} ***")
            ...
            # ── S — Stop & Save (RECORDING only) ─────────────────────────────
                ...
                if n == 0:
                    node.get_logger().warning(
                        "[Recorder] No data recorded — nothing saved.")
                else:
                    ...
                    node.get_logger().info(
                        f"[Recorder] Episode {episode_idx} saved ({n} steps). "
                        "Ready for next.")
                    episode_idx += 1
                reset_buf()
            ...
            # ── Q — Quit (any state) ──────────────────────────────────────────
                ...
                if rec_state == RECORDING:
                    node.get_logger().warning(
                        "[Recorder] Quit while recording — data NOT saved.")
            ...
            if rec_state == RECORDING:
                ...
                if now - last_step_t >= dt:
                    ...
                    if snap is not None:
                        episode_buf["cam1_rgb"].append(
                            preprocess_image(snap["cam1_rgb"]))
                        episode_buf["cam2_rgb"].append(
                            preprocess_image(snap["cam2_rgb"]))
                        ...
                        if len(step_ts) >= max_steps:
                            rec_state = WAITING
                            path = os.path.join(
                                dataset_dir, f"episode_{episode_idx}.hdf5")
                            node.get_logger().info(
                                f"[Recorder] Max steps ({max_steps}) reached "
                                "— auto-saving …")
                            save_episode_hdf5(
                                episode_buf, path, prompt, args.task, hz)
                            print_dt_diagnosis(step_ts, hz)
                            episode_idx += 1
                            reset_buf()
