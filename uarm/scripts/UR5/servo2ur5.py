...
class WeissCRGGripper:
    ...
    def _recv_all(self, n: int) -> bytes:
        ...
        while len(buf) < n:
            ...
            if not chunk:
                raise TimeoutError(
                    f"Serial read timeout (expected {n} bytes, got {len(buf)})")
            buf += chunk
        return buf
...
class UR5TeleopNode(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(Float64MultiArray, '/servo_angles',
                                 self._servo_callback, 10)
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            self.get_logger().debug(
                f"[UR5Teleop] action: {[f'{v:.2f}' for v in action_data]}")
