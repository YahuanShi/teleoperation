...
Layout: 4 rows × 2 cols (8 slots, 7 used)
    J0  J1
    J2  J3
    J4  J5
    Gripper  [empty]
    ...
    def _cb_rec(self, msg):
        with self._lock:
            ...
            if msg.data and not was:
                self._t_buf.clear()
                for b in self._uarm_buf: b.clear()
                for b in self._ur5_buf:  b.clear()
...
def run_plot(node: TrajVizNode):
    ...
    for i, ax in enumerate(axes):
        ...
        ax.set_title(CHANNEL_NAMES[i], fontsize=11, color="#aaaaaa",
        ...
        ax.legend(fontsize=8, loc="upper left", ncol=2,
                  handlelength=1.4, columnspacing=0.8, borderpad=0.5)

        vt = ax.text(0.99, 0.97, "", transform=ax.transAxes,
                     fontsize=8, color="#ffcc44", ha="right", va="top",
                     fontfamily="monospace")
        val_texts.append(vt)

    title_text  = fig.text(0.5, 0.965,
        "Joint & Gripper Trajectories   |   Blue: U-Arm    Red: UR5",
        ha="center", fontsize=12, color="#cccccc", fontweight="bold")
    status_text = fig.text(0.5, 0.012,
        ...
        for i in range(7):
            ...
            if i < 6:
                val_texts[i].set_text(
                    f"U:{cur_servo[JOINT_MAP[i]]:+.1f}°  R:{cur_state[i]:+.1f}°"
                )
            else:
                val_texts[i].set_text(
                    f"U:{cur_servo[6]:+.1f}°  R:{cur_state[6]*100:.0f}%"
                )
        ...
        if rec:
            ...
            if n > 1:
                ...
                status_text.set_text(
                    f"WAITING  —  last episode: {n} samples  {dur:.1f} s"
                )
            else:
                status_text.set_text(
                    "WAITING  —  press B in the recorder window to start recording"
